...
events = {
    '2001-10-06': 'this is a test tweet to see if `def tweet_historicat_event():` works. If you see this, @vicki_langer is super proud, overjoyed, and thankful for @bobbywin16\'s contribution',
    ...
    '1978-07-26': 'First LGBT + march in Mexico. https://www.gob.mx/cultura/es/articulos/breve-historia-de-la-primera-marcha-lgbttti-de-mexico?idiom=es'
    ...
    '2009-12-20': 'Mexico DF legalizes same-sex marriage. https://elpais.com/diario/2009/12/22/sociedad/1261436409_850215.html'
    '2010-07-15': 'Argentina legalizes same-sex marriage. https://es.wikipedia.org/wiki/Matrimonio_entre_personas_del_mismo_sexo_en_Argentina'
    '2010-08-18': 'Mexico legalizes homoparental adoption. https://es.wikipedia.org/wiki/Adopci%C3%B3n_homoparental#M%C3%A9xico'
    ...
    '2015-02-05': 'Law is enacted for gender identity of Trans people in Mexico. http://data.copred.cdmx.gob.mx/home/el-reconocimiento-de-la-identidad-de-genero-en-la-cdmx-previene-la-discriminacion-a-personas-trans/'
    ...
    '2015-12-17': 'Congress of Nayarit, Mexico approves equal marriage. https://www.sdpnoticias.com/diversidad/igualitario-matrimonio-congreso-nayarit-aprueba.html'
    '2016-04-28': 'Colombia legalizes same-sex marriage. https://es.wikipedia.org/wiki/Matrimonio_entre_personas_del_mismo_sexo_en_Colombia'
    ...
    '2019-06-12': 'Ecuador legalizes same-sex marriage. https://es.wikipedia.org/wiki/Diversidad_sexual_en_Ecuador'
...
def tweet_historicat_event():
    ...
    list_of_history_tweets = [key + ": " + val for key,
                              val in events.items() if month_and_day in key]
    ...
    if length_list_history_tweets != 0:
        if length_list_history_tweets > 1:
            history_tweet = tweet_template + \
                random.choice(list_of_history_tweets)
        else:
