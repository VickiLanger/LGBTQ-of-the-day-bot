...
# took this specific date as the example .. you can replace it with date.today() if events is upto date for 2021
today = "2004-07-20"

month_and_day = today[5:]  # slice off the first 5 characters

list_of_history_tweets = [key[5:] + ":\n" + val for key, val in events.items() if month_and_day in key[5:]]

    # here key is the whole "YYYY-MM-DD" value because it was
    #being stored through events.items()
    # you needed to make key[5:] while checking and storing 
    # for the correct implementation
    
length_list_history_tweets = len(list_of_history_tweets)

print(list_of_history_tweets)
